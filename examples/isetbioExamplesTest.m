function testResults = isetbioExamplesTest(scriptName)
% isetbioExamplesTest - Runs all s_* and t_* scripts in the examples directory safely
%
% Usage:
%   testResults = isetbioExamplesTest;
%   testResults = isetbioExamplesTest(scriptName);
%
...
% Define the target directory relative to isetbio root
targetDir = fullfile(isetbioRootPath, 'examples');
repoRoot = isetbioRootPath;
dependencyRoot = fullfile(fileparts(repoRoot), 'isetcam');

localEnsureRepoPath(repoRoot, 'ieSessionGet');
localEnsureDependencyPath(dependencyRoot, 'ieInit');
...
if isempty(allFiles)
    if isempty(scriptName)
        ...
        error('No example matched "%s" in %s.', scriptName, targetDir);
    end
    testResults = struct('file', {}, 'status', {}, 'error', {});
    return;
end

logInfo = ieInitTutorialExampleRunLog(repoRoot, 'isetbioExamplesTest', targetDir, allFiles, scriptName);
...
testResults = localRunAllFiles(allFiles, targetDir, 'isetbioExamplesTest', existingFigures, logInfo);
localPrintSummary('isetbioExamplesTest', testResults);
...
function localEnsureDependencyPath(dependencyRoot, requiredSymbol)
%% Add a dependency repo to the path when the required symbol is missing.

if isempty(which(requiredSymbol)) && exist(dependencyRoot, 'dir')
...
for fileIndex = 1:numel(allFiles)
    ...
    if localShouldSkip(filePath, skipPatterns) || localFileHasSkipTag(filePath)
        fprintf('SKIPPED\n');
