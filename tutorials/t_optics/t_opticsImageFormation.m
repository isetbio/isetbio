...
%
% Description:
%    This is a tutorial on optics and image formation spanning over twenty
%    years of effort.
%
% Notes:
%    * [Note: DHB - This is a really nice tutorial and it runs. But it
%      relies on data that are live under the scripts/optics directory. We
%      need to decide how we're going to handle those .mat files, which are
%      not in ISET data format. It would be nice to conver them to be so,
%      and make the idiosyncratic data go away.]
%

% History:
%    01/02/96       Created. Duration 45M
%    01/06/98  BW   Checked against Matlab 5 by BW.
%    01/07/99  BW   Added IjSpeert calls, BW
%    01/12/01  BW   Added new graphs for the ijspeert curves and Williams
%                   et al. data, BW
%    01/18/03  XXX  Started integration with vCamera-2.0, minor changes.
%    01/02/08  BW   Verified with local files for single download - BW
%    06/29/15  XXX  Don't use rad2deg and deg2rad as variable names, since
%                   these are Matlab functions.
%    09/11/18  jnm  Formatting updates.
...
%% Calculations
% In radians, the viewing angle, phi, satisfies the equation
...
%
% Suppose we wish to plot the function by defining the spatial variable, x,
% in terms of seconds of arc,
xSec = -300:1:300;	
xMin = xSec / 60;
...
vcNewGraphWin;
plot(xSec, ls)
set(gca, 'xlim', [-240 240], 'xtick', (-240:60:240))
grid on;
xlabel('Arc sec');
ylabel('Responsivity');
title('Westheimer Linespread')
...
% image. We can compute the retinal image by forming the convolution of the
% image with the Westheimer linespread function. Remember: we sampled the
% linespread once every sec of arc. So, we can simply convolve the image
% and the linespread function now as:
...
% (If the colors in the image look funny, make sure to place your cursor
% inside the window. This may change the local color map).
...
%
% The below fspecial will produce a little Gaussian window for filtering.
...
% be detectable by the observers?  How can we tell? You might also ask what
% will happen when we view the page at 6 inches, or at 24 inches. What if
% we increase the printer resolution to 1200 dpi?  What if we introduce
% some ability to modulate the density of the ink and hence the light
% scattered back to the eye?

%% Defocus in the frequency domain
% First, make a new linespread function that is smaller and easier to
% compute with. Have it extend over 1 deg (60 min) so the Fourier Transform
...
grid on;
xlabel('Min of arc');
ylabel('Linespread value');

% Now, consider the retinal image formed by some simple harmonic functions.
% Here is a sinusoid that varies at 1 cycle per degree of visual angle.
...
% frequency increases. We will store the amplitude of the cosinusoid in the
% variable "peak".
vcNewGraphWin;
freq =[1 5 10 15];
...
%% Compare convolution and Fourier Transform of the linespread function
% Remember, the linespread was built so that it spans 1 deg, hence
% frequency is in cycles per degree.
mtf = abs(fft(ls));
hold on;
plot(freq, mtf(freq + 1), 'ro');
hold off;

% The values we obtain from convolution are plotted as solid line, whereas
% the amplitude of the Fourier Transform of the linespread function is
% plotted as a red circles at each frequency.

% The functions match, which should give you some intuition about what the
% amplitude of the Fourier Transform represents.

%% Comparison of the pointspread and linespread
% When working with two-dimensional inputs, we must consider the
% pointspread function, that is the response to an input that is a point of
% light. A standard formula for the cross-section of the pointspread
% function of the human eye for a 3mm pupil is also provided by Westheimer.
% We can compare the linespread and the cross-section of the pointspread in
% the following graphs.
xSec = -300:300;
xMin = xSec / 60;
ls = 0.47 * exp(-3.3 * (xMin .^ 2)) + 0.53 * exp(-0.93 * abs(xMin));
ps = 0.952 * exp(-2.59 * abs(xMin) .^ 1.36) + 0.048 ...
    * exp(-2.43 * abs(xMin) .^ 1.74);

vcNewGraphWin;
p = plot(xSec, ps, 'r-', xSec, ls, 'b--');
grid on;
set(gca, 'xlim', [-180 180]);
xlabel('Arc sec');
ylabel('LS or PS amplitude');
legend(p, 'Pointspread', 'Linespread');

% Next, we can create a graph of the pointspread. First, create a matrix
% whose entries are the distance from the origin
xSec = -240:10:240;
xMin = xSec / 60;
...
% If you want to see the distance from the origin, show this image:
%{
    colormap(gray(32))
    imagesc(D)
    axis image
...
% good focus at 580 nm (yellow-part of the spectrum) the light in the
% short-wavelength (400-450nm) is blurred quite strongly and light in the
% long-wavelength part of the spectrum is blurred, too, though somewhat
% less. We can calculate the linespread as a function of wavelength
% (Marimont and Wandell, 1993) from basic principles. The linespreads a
% various wavelengths are contained in the data file:
oi = oiCreate('human');
...
% We select three wavelengths and plot their linespread functions together.
% Notice that for the shorter wavelength, the linespread function is much
% more spread-out than for the middle and long wavelengths.
vcNewGraphWin;
...
% Different wavelength components of an image are blurred to different
% extents by the eye. We use a set of lines again as an example. For this
% computation, we will assume that the input begins with equal energy at
% all wavelengths from 370 to 730 nm.
...
% To calculate the retinal image for this pattern, convolve each wavelength
% component of the image with the appropriate linespread function. The
% routine conv2 takes the input image (im, size(im) = 1 128) and convolves
% it with each of the linespread functions in lineSpread size(lineSpread =
% 361, 65). This results in 361 images (one for each wavelength).
...
% We can plot the retinal image corresponding to two wavelengths this way.
vcNewGraphWin;
subplot(2, 1, 1)
plot(X, retIm(201, :), 'g-');
set(gca, 'ylim', [0 0.5]);
...
% Notice the two images have the same mean, and only differ in terms of the
% contrast: the green image has a lot more contrast, but the same mean.
mean(retIm(50, :), 2)
mean(retIm(200, :), 2)
...
% the eye's optical system for individual wavelengths. For short wavelength
% lights, high spatial frequency contrast is attenuated a lot by the
% optical path of the eye.

% [Note: XXX - *OLD* - Load the MTFs for wavelengths from 370-730nm. These
% were calculated using the methods in Marimont and Wandell that are in a
% script in the /local/class/psych221/tutorials/chromAb sub-directory.
% load combinedOtf; - *OLD*]
...
% in the average human eye. The curves they derived were based on empirical
% inspection of data sets, and do not have any strong theoretical
...
% A student in Psych 221 developed the code to compute these values in the
% function "ijspeert."  Here is an example:
age = 20; 				    % Subject's age
pupil = 1.5; 				% diameter in mm
pigmentation = 0.142; 		% Caucasian
freqIndexRange = 1:50; 		% The spatial frequency range

% Set the span to be 1 deg, so we know that 1 cycle in the MTF corresponds
% to 1 cycle per deg
...
% The functions should be normalized so that the area under the linespread
% and the first value of the MTF are one.
iMTF = iMTF / iMTF(1);
iLSF = iLSF / sum(iLSF);

% These are the modulation transfer function and linespread for the
% ijspeert data
vcNewGraphWin;  % clf
subplot(1, 2, 1)
plot(iMTF);
...
% The PSF is circularly symmetric. So, we can accumulate the 1D values into
% a 2D surface.
%
angleInRad2D = linspace(min(angleInRad) / 4, max(angleInRad) / 4, ...
    length(angleInRad) / 4);
nSamples = length(angleInRad2D);
[X, Y] = meshgrid(angleInRad2D, angleInRad2D);

% Because the pointspread function is symmetric, we can calculate the value
% by building a matrix D that measures only the distance from the center of
% matrix. Try plotting this distance matrix using the command:
%{
    imagesc(D);
	colorbar;
    axis image
...
% Now, loop through the rows of D to calculate the pointspread values as a
% function of angle.
iPSF2D = zeros(size(D));
for ii = 1:nSamples
   a = D(ii, :);
