...
% The commented out line generates a warning to use the computeCurrent
% method.  So I broke the calculation out into two parts.  But, now I get a
% message (neither a warning nor an error) that says:
% "You must compute isomerizations (absorptions) time series prior to the
% current."  Not at all clear how to fix, from any documentation I can
% easily locate.
%
%[isomerizationsMap, currentsMap] = cMosaicOBJ.compute(oi,'currentFlag', true);
isomerizationsMap = cMosaicOBJ.compute(oi);
currentsMap = cMosaicOBJ.computeCurrent;
