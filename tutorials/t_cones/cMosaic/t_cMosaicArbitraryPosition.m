...
center = [10 10];
cmfov = [2 2];
cmM = cMosaic('eccentricityDegs',center,'sizeDegs',cmfov);

% Compute the response, placing the OI at the center of the mosaic
noiseFreeMosaicCentered = cmM.compute(oi, 'opticalImagePositionDegs', 'mosaic-centered');

% Compute the response, placing the OI at (x=10.3, y=9.5);
noiseFreeOffCentered1 = cmM.compute(oi, 'opticalImagePositionDegs', [10.3 9.5]);

% Compute the response, placing the OI at (x=9.5, y=9.5);
noiseFreeOffCentered2 = cmM.compute(oi, 'opticalImagePositionDegs', [9.5 9.5]);
...
% The mosaic
ax = subplot(2,2,1);
cmM.visualize('figureHandle', hFig, 'axesHandle', ax, ...
