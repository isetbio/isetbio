...
%
% Description:
%     Illustrates the various ways to generate noise using the coneMosaic object.
%     We set coneMosaic.noiseFlag to 'random', 'frozen', and 'none'.
%
%     This tutorial also illustrates how to access some of the key fields of the coneMosaic
%     object programatically and make some plots of what is in there.
%
...
%
% In ISETBio, we do not add Poisson noise to the retinal image
% by default, but rather take the mean value and let the noise
% emerge whene we compute isomerizations.
...
%% View the mosaic in a window.  You can see that the isomerizations look noisy.
%
% If you want, in the window, select Plot:hLine LMS and
% then click on a row.  Up will come a plot showing the L, M and S cone
% isomerizations in separate subplots, and you will observe that they look
% noisy.  Just below, we'll make that type of plot programatically.
cMosaic.window;
...
% but it is not there currently.  The code below was adopted from
% the plotting engine used by coneMosaic.window, look in file coneMosaicWindow.
%
...
whichLine = 15;
yLabelConeNames = ['L', 'M', 'S']; yLabelCommon = 'Cone Isomerizations';
plotSpec = {'ro-','go-','bo-'};
for ii = 2:4 
    subplot(3, 1, ii-1);
    positionIndex = find(cMosaic.pattern(whichLine, :) == ii);
    plot(positionIndex, cMosaic.absorptions(whichLine, positionIndex), plotSpec{ii-1}, 'LineWidth', 2);
    grid on;
...
% absorption by the lens and macular pigments. The smaller number of isomerizations
% means that the Poisson noise has a larger relative effect for the S cones.
cMosaic.compute(oi);
figure(linePlot1); 
plotSpec = {'ro:','go:','bo:'};
for ii = 2:4 
    subplot(3, 1, ii-1); hold on
    positionIndex = find(cMosaic.pattern(whichLine, :) == ii);
    plot(positionIndex, cMosaic.absorptions(whichLine, positionIndex), plotSpec{ii-1}, 'LineWidth', 2);
end
...
%
% This returns the mean number of isomerizations, without Poisson
% noise.  Look how beautifully smooth these functions are.
...
% Save the mean isomerizations from the no noise case just above, for plotting.
...
plot(meanIsomerizations(:),frozenIsomerizations1(:),'ro','MarkerFaceColor','r','MarkerSize',8);
plot(meanIsomerizations(:),frozenIsomerizations2(:),'ko','MarkerFaceColor','k','MarkerSize', 4);
xlabel('Mean Isomerizations');
ylabel('Frozen Isomerizations');
title('Two Draws of Frozen Isomerizations Against Mean');
legend({'First Frozen Draw','Second Frozen Draw'},'Location','NorthWest');
axis('square');
