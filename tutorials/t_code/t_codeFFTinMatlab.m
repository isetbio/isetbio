% Explain Matlab conventions for transforming space domain to frequency
%
% Description:
...
% floor(N / 2) + 1. So if N = 4, the center is (3, 3) and if N = 5, the
% center is also at (3, 3).
%
% When we pad an image or a filter, we want to do so in a way that the
% value at the center remains at the center.
%
% Suppose we have an even size image, say N = 6, and we pad it to N = 7.
% The old center was at (4, 4) and the new center will be at (4, 4). To
% preserve the old center location, pad at the bottom and right first.
%
% From 7 to 8, the old center was at (4, 4) and the new center will be at
% (5, 5). So for this transition, we should pad at the top and left.
...
if (max(abs(imgFT(:)) - abs(imgFTNoShift(:))) > 1e-10)
    fprintf(strcat("Surprising difference in FFT modulus with ", ...
        "insertion of ifftshift\n"));
else
    fprintf(strcat("FFT modulus behaves as expected with insertion ", ...
        "of ifftshift\n"));
end
if (max(angle(imgFT(:)) - angle(imgFTNoShift(:))) > 1e-10)
    fprintf(strcat("Expected difference in FFT phase with insertion ", ...
        "of ifftshift\n"));
else
    fprintf(strcat("FFT phase unexpectedly preserved with insertion ", ...
        "of ifftshift\n"));
end
