...
function [spectrum, displacement] = performAnalyses(fixEMobj)
    ...
    spectrum.powerSpectralDensityX = squeeze(mean(powerSpectralDensityX,1));
    spectrum.powerSpectralDensityY = squeeze(mean(powerSpectralDensityY,1));
    ...
    displacement.xScrambledIntervals = squeeze(mean(scrambledIntervalsDisplacementX,1));
    displacement.yScrambledIntervals = squeeze(mean(scrambledIntervalsDisplacementY,1));
    ...
    displacement.xD2degsScrambedIntervals = scrambledIntervalsDisplacementX2/(60^2);
    displacement.yD2degsScrambedIntervals = scrambledIntervalsDisplacementY2/(60^2);
    displacement.D2degsScrambedIntervals = displacement.xD2degsScrambedIntervals + displacement.yD2degsScrambedIntervals;
end
...
function hFig = visualizeAnalysis(fixEMobj, spectrum, displacement, figureName)
    ...
    % Plot the results
    hFig = figure(); clf; 
    set(hFig, 'Name', figureName, 'Position', [0 0 1150 950], 'Color', [1 1 1]);
    ...
    % Velocity (last trial)
    subplot(3,3,[4 5]);
    plot(fixEMobj.timeAxis, squeeze(fixEMobj.velocityArcMin(end,:)), 'k-', 'LineWidth', 1.5);
    hold on;
    plot(fixEMobj.timeAxis, squeeze(mean(fixEMobj.velocityArcMin,1)), 'k-', 'Color', [0.9 0.8 0.2], 'LineWidth', 3.0);
    subjectDR.mean = 30;
    subjectDR.std = 17;
    ...
    hold off;
    grid on
    legend({'model (one iteration)', 'model (mean across all iterations)', 'Cherici et al: subjectDR', 'Cherici et al: subjectDG'}, 'Location', 'NorthEast')
    set(gca, 'XLim', [fixEMobj.timeAxis(1) fixEMobj.timeAxis(end)], ...
    ...
    subplot(3,3,7);
    plot(spectrum.frequencyAxis,10*log10(spectrum.powerSpectralDensityX), 'r-', 'LineWidth', 1.5); hold on
    plot(spectrum.frequencyAxis,10*log10(spectrum.powerSpectralDensityY), 'b-', 'LineWidth', 1.5); hold off
    ...
    % Displacement^2 plot
    subplot(3,3,8);
    plot(displacement.timeLagsMilliseconds, displacement.D2degs, 'k-', 'LineWidth', 1.5); hold on;
    plot(displacement.timeLagsMilliseconds, displacement.D2degsScrambedIntervals, 'k--', 'LineWidth', 1.5); hold off;
    grid on
    minD2 = min([min(displacement.D2degs(:)) min(displacement.D2degsScrambedIntervals(:))]);
    D2range = [...
        minD2 ...
        minD2 * 1000 ...
        ];
    set(gca, 'XLim', [1 1000], 'YLim', D2range, 'XTick', [1 3 10 30 100 300 1000], ...
    ...
    subplot(3,3,9);
    plot(displacement.timeLagsMilliseconds, displacement.x, 'r-', 'LineWidth', 2); hold on;
    plot(displacement.timeLagsMilliseconds, displacement.y, 'b-', 'LineWidth', 2);
    plot(displacement.timeLagsMilliseconds, displacement.xScrambledIntervals, 'k--', 'LineWidth', 1.5); hold off;
    grid on
    minD = min([min(displacement.x(:)) min(displacement.xScrambledIntervals(:))]);
    Drange = [...
        minD ...
        minD * 100 ...
        ];
    
    set(gca, 'XLim', [2 1000], 'YLim', Drange, 'XTick', [3 10 30 100 300 1000], ...
