...
%% Stimulus params. 
% Here we are specifying a square stimulus that is
...
emPlot(fixEMobj,'emMosaic','cone mosaic',cm,'visualized trial',visualizedTrial);
...
% 2D activation maps
subplot(3,6,[13 18]);
plot(fixEMobj.timeAxis, squeeze(fixEMobj.emPosMicrons(visualizedTrial,:,1)),...
    'r-', 'LineWidth', 2); 
hold on
plot(fixEMobj.timeAxis, squeeze(fixEMobj.emPosMicrons(visualizedTrial,:,2)),...
    'b-', 'LineWidth', 2);
plot(fixEMobj.timeAxis, 0*fixEMobj.timeAxis, 'k-', 'Color', [0.4 0.4 0.4],...
    'LineWidth', 1.0);
% Superimpose stimulus frames
for k = 1:numel(stim.timeAxis)
    plot(stim.timeAxis(k)*[1 1], stim.modulation(k)*[-100 100], 'k-', ...
