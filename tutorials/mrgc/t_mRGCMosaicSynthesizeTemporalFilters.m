...
% Examples:
%{
	BenardeteKaplanScreenRefreshRate = 135;
    examinedTemporalFrequencies = [1.4 2 3 4 6 8 12 16 24 32];
    spotStimulusPositionDegs = [-6.081 -0.047];
    spotStimulusDiameterDegs = 0.05;
    spotStimulusContrast = 0.3;

    % High-res run
    BenardeteKaplanScreenRefreshRateHz = 400;
    tfMinHz = 0.4;
    tfDeltaHz = 2.0;
    tfMaxHz = BenardeteKaplanScreenRefreshRateHz/2;

    examinedTemporalFrequencies = tfMinHz : tfDeltaHz : tfMaxHz;
    spotStimulusPositionDegs = [-6.081 -0.047];
    spotStimulusDiameterDegs = 0.04;
    spotStimulusContrast = 0.5;
    ...
    mRGCNonLinearityParamsStruct = struct(...
                ...
                'pCurrentTemporalResolutionSeconds', 2.5/1000 ...
    );

	% Compute input cone mosaic excitations responses
	t_mRGCMosaicSynthesizeTemporalFilters(...
       'rgcMosaicName', 'PLOSpaperTemporal7DegsMosaic', ...
       'stimulusPositionDegs',  spotStimulusPositionDegs, ...
       'stimulusSizeDegs', spotStimulusDiameterDegs, ...  		% Spot TTF (0.25 degs in diameter)
       'stimulusShape', 'spot', ...
       'temporalPhaseIncrementDegs', -1000/BenardeteKaplanScreenRefreshRate, ...
       'frequencySupport', examinedTemporalFrequencies, ...
       'chromaticity', 'Achromatic', ...
       'contrast', spotStimulusContrast, ...
    ...
    % Compute input cone mosaic photocurrent responses
	t_mRGCMosaicSynthesizeTemporalFilters(...
       'rgcMosaicName', 'PLOSpaperTemporal7DegsMosaic', ...
       'stimulusPositionDegs',  spotStimulusPositionDegs, ...
       'stimulusSizeDegs', spotStimulusDiameterDegs, ...        % Spot TTF (0.25 degs in diameter)
...
arguments
	...
	% If less than 0, it is interpreted as the temporal resolution of the stimulus (frame duration in msec)
    options.temporalPhaseIncrementDegs (1,1) double = [];
...
if (computeInputConeMosaicResponses || inspectInputConeMosaicResponses)
	...
	RGCMosaicAnalyzer.compute.mosaicTTFsForStimulusChromaticityAndOptics(...
        ...
        'inputConeMosaicSizeFraction', 0.75, ...
