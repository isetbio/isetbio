%% t_sceneFromSurface
% 
% Show how to create a scene from scratch, starting with surface
...
%
% This could be anything, but here we load in the
% Macbeth Color Checker Chart and expand it out
% so we can see it.
%
% Make this an m x n x nwave matrix and look at it to make
% sure we didn't muck it up.
%
% Routine imageSPD is a utility that will render spectral data
% and show it as an image in a new window. 
patchSize = 12;
...
% The leading constant of 683 puts us in cd/m2
% for energy radiance in Watts/[m2-sr-nm]
...
for i = 1:nRows
    for j = 1:nCols
        radianceEnergy(i,j,:) = theIlluminant.*squeeze(macbethReflectance(i,j,:));
    end
end
