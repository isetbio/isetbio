function sensor = sensorHorwitzHassShortWhiteNoise(params, scene, oi, display)
...
%     end
    ...
    if t < 10 %nSteps / 4
        scene = sceneAdjustLuminance(scene, 200 * (t/(10)) );
%     elseif (t >= nSteps / 4) && (t <= 3 * nSteps / 4)
    elseif (t >= 10) && (t <= 20)
        scene = sceneAdjustLuminance(scene, 200);
    elseif t > 20 %3 * nSteps / 4
        scene = sceneAdjustLuminance(scene, 200 * (((10+10+10) - t)/30) );
    end
