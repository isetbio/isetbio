...
% 9/10/21  dhb  Respect BLTB preference 'CalDataFolder' if it exists, is not empty, and
%               calDir is not passed.
...
if (nargin < 3 || isempty(calDir))
    ...
    % Don't have a dir.  First check BLTB preference
    if (ispref('BrainardLabToolbox','CalDataFolder'))
        calDir = getpref('BrainardLabToolbox','CalDataFolder');
    end
    
    % If it is still empty, try defaults
    if (isempty(calDir))
