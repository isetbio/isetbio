...
void do_predict(FILE *input, FILE *output)
{
	...
	while(readline(input) != NULL)
		total++;
	rewind(input);
	dvec_t = Malloc(double,total);
	ivec_t = Malloc(double,total);
	query = Malloc(int,total);
	total = 0;
	while(readline(input) != NULL)
	{
		int i = 0;
	...
	if (model_->param.solver_type==L2R_L2LOSS_RANKSVM)
	{
		double result[2];
	...
	else if(model_->param.solver_type==L2R_L2LOSS_SVR ||
