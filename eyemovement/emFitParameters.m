...
%
% Inputs:
%    fileName  - Varies. This is either a file name for a data file
%                containing an eye movement matrix, or a matrix of eye
%                movement data.
%
% Outputs:
%    mu        - Vector. 2 element vector containing the mean position.
%    Sigma     - Matrix. 2-by-2 matrix, covariance matrix of eye movement.
%
...
xMax = quantile(xPos, 0.99);
xMin = quantile(xPos, 0.01);
yMax = quantile(yPos, 0.99);
yMin = quantile(yPos, 0.01);
...
%    discontinuity in xPos and yPos. This will lead to extreme value in
%    xDiff and yDiff
xMax = quantile(xDiff, 0.99);
xMin = quantile(xDiff, 0.01);
yMax = quantile(yDiff, 0.99);
yMin = quantile(yDiff, 0.01);
