...
%RGCmosaicName = 'JCNpaperTemporal7DegsMosaic';   % Fully done. Optimizing L cone on Crete now
%RGCmosaicName = 'JCNpaperTemporal4DegsMosaic';   % Fully done. Also done with second  subject optics
RGCmosaicName = 'JCNpaperTemporal2DegsMosaic';   % Fully done. Last one Computing input cone mosaic on Leviahtnan now
...
if ((~reAnalyzeSTFData) && (aggregatePreviouslyAnalyzedRunsFromMultipleTargetVisualSTFs||aggregatePreviouslyAnalyzedRunsFromMultipleEccentricities))
    ...
    elseif (aggregatePreviouslyAnalyzedRunsFromMultipleEccentricities)
        ...
        % Fully done for 'JCNpaperDefaultSubject', which is subject #2(ranked #3), and 
