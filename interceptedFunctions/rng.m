...
function settings = rng(arg1,arg2)
    ...
    if (isfield(rngTrackingInfo, 'scenarioBeingRun'))
        ...
        switch (nargin)
            case 0
                dialogHeader = generateDialogHeader();
                updateUI();

            case 1
                dialogHeader = generateDialogHeader(arg1);
                updateUI(arg1);

            case 2
                dialogHeader = generateDialogHeader(arg1,arg2);
                updateUI(arg1, arg2);
        end
...
function updateUI(arg1, arg2)
    ...
    if (~isempty(theUIFigure))
        ...
        switch (nargin)
            ...
            case 1
                if (ischar(arg1))
                    lbl3.Text = sprintf('Rng() call (#%d) with a single argument (''%s'')', rngCallNo, arg1);
                elseif (isnumeric(arg1))
                    lbl3.Text = sprintf('Rng() call (#%d) with a single argument (%g)', rngCallNo, arg1);
                elseif (isstruct(arg1))
                    lbl3.Text = sprintf('Rng() call (#%d) with a struct argument', rngCallNo);
                else
                    lbl3.Text = sprintf('Rng() call (#%d) with an argument that is not a char, a numeric, or a struct.', rngCallNo);
                end

            case 2
                if (isnumeric(arg1)&&isnumeric(arg2))
                    lbl3.Text = sprintf('Rng() call (#%d) with %d arguments (%g, %g)', rngCallNo, nargin, arg1, arg2);
                else
                    lbl3.Text = sprintf('Rng() call (#%d) with %d arguments that are not numerical', rngCallNo, nargin);
                end
...
function dialogHeader = generateDialogHeader(arg1, arg2)
