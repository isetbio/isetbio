function varargout = v_ibio_sceneReIllumination(varargin)
%
...
function ValidationFunction(runTimeParams)   
    ...
    UnitTest.validationData('scene', scene, ...
        ...
        'scene.data.photons',1e10, ...
        'scene.illuminant.data.photons',1e10, ...
        'scene.data.meanL',1e-5);   
...
function plotRadianceMap(radianceMap, wavelengthSampling, wavelengthSubSamplingInterval, titleText)
    [X,Y,Z] = meshgrid(1:size(radianceMap,2), double(wavelengthSampling), double(1:size(radianceMap,1)));
    ...
    w = double(wavelengthSampling(1):wavelengthSubSamplingInterval:wavelengthSampling(end));

    h = slice(X,Y,Z, radianceMap, Inf, w, Inf, 'nearest');
