...
%
% Syntax:
%   yi = quinterp2(X, Y, Z, xi, yi, [methodFlag])
%
% Description:
%    qinterp2 provides a speedup over interp2 in the same way that
%    qinterp1 provides a speedup over interp1. It follows the same usage as
%    interp2, where X and Y should be "plaid" (e.g. generated with
%    meshgrid). The Default is bilinear interpolation(2). 
%
% Inputs:
%    X          - X portion of X-Y "plaid"
%    Y          - Y portion of X-Y "plaid"
%    xi         - X portionn of xi-yi "plaid"
%    yi         - y portionn of xi-yi "plaid"
%    methodFlag - Method flag to determine type of interpolation, Options:
%        0 - Nearest-neighbor 
%        1 - Triangular-mesh linear interpolation.
%        2 - Bi-linear (equivalent to MATLAB's 'linear')
%
% Outputs:
%    Zi         - Interpolation
%
...
%    * Error checking
%       - *WARNING*: Little error checking is performed on the X or Y
%         arrays. If these are not proper monotonic, evenly increasing
%         plaid arrays, this function will produce incorrect output without
%         generating an error. This is done because error checking of the
%         "library" arrays takes O(mn) time (where the arrays are of the
%         size [m, n]). This function is algorithmically independent of the
%         size of the library arrays, and its run time is determine solely
%         by the size of xi and yi.
%       - Imageval:  We added creation of the meshgrid version of vectors
...
%
% See Also:
%   quinterp1 (using with non-evenly spaced arrays), interp2
%
...
switch method
    ...
    % Linear method
    case 1
        % Split the square bounded by (x_i, y_i) & (x_i + 1, y_i + 1) into
        % two triangles. The interpolation is given by finding the function
        % plane defined by the three triangle vertices
        fxi = floor(xi) + 1;
        ...
        % Indices of (x_i + 1, y_i)
        ind2 = fyi + librarySize(1) * fxi;
        % Indices of (x_i + 1, y_i + 1)
        ind3 = fyi + 1 + librarySize(1) * fxi;
        % Indices of (x_i, y_i + 1)
        ind4 = fyi + 1 + librarySize(1) * (fxi - 1);
        ...
        % flagCompare determines which triangle the requested loc. is in
