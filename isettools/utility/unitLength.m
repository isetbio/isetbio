...
%    * [Note: XXX - Look at Kendrick's function below for ideas. In
%      particular, he allows specifying whether we want this done for rows
%      or columns In the future we will add a parameter, dim = 1 through n
%      to define which dimension we want to be unit length. For now, He
%      also uses that bsx stuff which is faster]
%
...
% %   if supplied, normalize each case oriented along <dim> to have
% %      unit length.
...
% %   weird cases where the length of <m> is very small to start with
% %      (see zerodiv.m).
% %   default: 1.
% % <sc> (optional) is a special case. supply this and we will use it
% %   instad of calculating the actual scale factor. also, <len> will be
% %   returned as []. to indicate not-supplied, pass in [].
% %
% % unit-length normalize <m> via scaling, operating either on individual
% % cases or globally. the output <f> has the same dimensions as <m>. also,
% % return <len> which is the vector length of <m> along <dim>. when <dim>
% % is [], <len> is a scalar; otherwise, <len> is the same dimensions as
% % <m> except collapsed along <dim>. also, return <sc> which is the scale
% % factor divided from <m>. the dimensions of <sc> is the same as <len>.
% %
...
% % else
% %   f = zerodiv(m, repmat(sc, copymatrix(ones(1, ndims(m)), dim, ...
% %       size(m, dim))), NaN, wantcaution);
% % end
