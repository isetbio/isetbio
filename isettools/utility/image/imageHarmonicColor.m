...
%    example, below. 
%
%    When some of the fields are vectors (freq, contrast, ang, phase) then
%    the return produces the sum of these harmonics. The sum always has a
%    mean of 1.
%
...
%    size is 128 and GaborFlag = 0.25, the standard deviation is 32.
%
...
%               'color'     - The color scheme. Default is 1.
%
% Outputs:
...
%
% Notes:
%    * [Note: JNM - Why is color set and then never used? (parms.color is
%      referenced in color's stead. Changed switch statement to use the
%      created color variable as appears to have been intended instead.]
...
try
    contrast = parms.contrast; 
catch
    contrast = 1;
    parms.contrast = contrast;
end
try
    freq = parms.freq;
catch
...
for rgbIndex = 1:3
    for ii = 1:length(freq)
        img(:, :, rgbIndex) = img(:, :, rgbIndex) ...
            + colorOpponent(rgbIndex) * contrast(ii) * g ...
            .* cos(2 * pi * freq(ii) * (cos(ang(ii)) * X ...
            + sin(ang(ii)) * Y) + ph(ii)) + 1;
    end
end
