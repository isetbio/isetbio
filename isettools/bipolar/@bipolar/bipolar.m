classdef bipolar < handle
    ...
    %
    %   bp = bipolar(os,varargin)
    %
    % The bipolar class simulates the retinal processing from the cone
    % outer segment current to the continuous current output of the
    % bipolar center and surround.
    %
    % We approximate this bipolar temporal filter with a weighted
    % differentiator, such that 
    %
    %  responseBipolar(t) = w1*responseCone(t) + w2*responseCone'(t), 
    %
    % where w1 is close to zero and w2 is a large negative number.
    % 
