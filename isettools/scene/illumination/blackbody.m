...
%
% Syntax:
%  [specRad, XYZ] = blackbody(wave, temps, [unitType], [eqWave])
%
% Description:
...
%    you request photon units (unitType = 'photons'), the returned values
%    will be in quanta/(m^2 nm sr).
%
%    The color temperatures are in degrees Kelvin. 
%
%    The returned spectra have a luminance on the order of 100 cd/m2. They
%    are scaled so that the radiance at the eqWave value (eqWave default
%    550) are equal.
%
...
%    blackbody.m' into the Command Window.
%
% Inputs:
%    wave     - The wavelength(s).
%    temps    - The color temperature(s) in degrees Kelvin.
%    unitType - (Optional) The unit type of energy. Default 'watts'.
%    eqWave   - (Optional) The wave at which point to return the radiance.
%               Default is 550 nm.
%
% Outputs:
%    specRad  - The calculated spectral radiance
%    XYZ      - The CIE XYZ values
%
...
% Other constants specific to the black body radiator formula & their units
...
% [Note: XXX - Apply Planck's Law. Now here is an obscure piece of code.
% Comes from DiCarlo at some point in time. Let's re-write this formula so
% a normal person can read it. It should be basically hV / (exp(hv / kT) -
% 1) where T are the temps, waveM is the wavelength, and so forth. Notice
% that we have the fundamental units above, but Jeff put in the needed
% products instead.]
