...
%
% Description:
%    Evaluate the linear fit L from ths sensor RGB to the ideal RGB of a
%    Macbeth Color Chart.
%
%    We convert the linear sRGB to sRGB and do the evaluations of the
%    observed and predicted RGB, various CIELAB error terms, and
%    chromaticity values.
%
%    Key data in the graph are stored in the figure and can be acquired
%    using get(gcf, 'userData')
%
% Inputs:
%	 L         - (Optional) Linear transform that maps the sensor rgb into
%                the linear rgb values of the MCC.  If L is identity, the
%                person already did the computation and we are just running
%                this routine for the graphic evaluation. Default eye(3, 3)
%    sensorRGB - Sensor RGB of the MCC in standard order where gSeries is
%                4:4:24.  Data are passed in as XW format.
%    idealRGB  - (Optional) Should get this from macbethIdealColor. In XW
%                format. Default is a 'D65' illuminated 'lRGB' color chart.
%    sName     - (Optional) The name of the sensor. Default is 'sensor'.
%
% Outputs:
%    figNum    - 
%
...
xlabel('Observed (r, g, b)');
ylabel('Desired (r, g, b)');
grid on
