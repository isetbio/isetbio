...
%
% Description:
%    Scene properties are either stored as parameters or computed from
%    those parameters. We generally store only unique values and to
...
%    'm', 'cm', 'mm', 'um', 'nm'. Default is always meters ('m').
%
%    From 'radiance and reflectance information' parameters section:
%    An roi can be N x 2 xy locs or a rect vector containing:
...
%         {'known reflectance'}      - A 4-vector, containing reflectance,
%                                      row, col, wave
%         {'peak radiance'}          - peak radiance at specified
%                                      (or all) wavelengths
...
%         {'roi photons'}            - spd of the points in a region of
%                                      interest The region can be a rect or
%                                      xy locs
%         {'roi energy'}             - as above, but energy
%         {'roi reflectance'}        - as above, but reflectance
%         {'roi mean energy'}        - energy spd averaged within a region
%                                      of interest 
%         {'roi mean photons'}       - photons spd averaged within in a
%                                      region of interest 
%         {'roi mean reflectance'}   - reflectance spd averaged within in a
%                                      region of interest
...
%                                     (do not use)
%         {'distance per degree'} *   - sample spacing/deg of visual angle
%         {'degrees per distance'}  - degrees per unit distance, e.g.,
...
%         {'frequency resolution'} *  - Default is cycles per degree, e.g.
%                                     val = sceneGet(scene, ...
%                                     'frequencyResolution', units);
%         {'maxfrequencyresolution'} * 
%                                   - Default is cycles/deg. By using, say, 
%                                     sceneGet(oi, 'maxfreqres', 'mm') you
%                                     can get units in cycles/{m, mm, um}
%         {'frequencysupport'} *      - val = sceneGet(scene, ...
%                                     'frequencyResolution', units);
%         {'fsupportx'} *             - val = sceneGet(scene, ...
%                                     'frequencyResolution', units);
%         {'fsupporty'} *             - val = sceneGet(scene, ...
%                                     'frequencyResolution', units);
...
%         {'illuminant'}         - HDRS multispectral data illuminant
%                                  stored here (watts/sr/m^2/nm)
...
%         {'roi illuminant photons'}
%                                - illuminant photon spd within a specific
%                                  region of interest 
%         {'roi mean illuminant photons'}
%                                - illuminant photon spd averaged within in
%                                  a region of interest
%      Display
%         {'rgb image'}  - RGB image of the scene display
%    varargin - (Optional) Most often the units for the parameters.
%
% Outputs:
...
%
% Notes:
%	 * [Note: BW - Why are we allowing empty scenes now?  validateFastAll
%      doesn't run if we don't allow them. I catch that case here and
%      return with val empty. If we don't catch the case here, closing
%      windows becomes a problem.]
%    * N.B. The source contains executable examples of usage, which can be
%      accessed by typing 'edit sceneGet.m' in MATLAB's command window.
%

% History:
%    xx/xx/03       Copyright ImagEval Consultants, LLC, 2003.
%    01/02/18  jnm  Formatting
...
switch parm
    ...
    case {'samplespacing'}
        ...
        val = [sceneGet(scene, 'width') / sz(2), ...
            sceneGet(scene, 'height') / sz(1)];
        if ~isempty(varargin)
    ...
    case {'distance', 'objectdistance', 'imagedistance'}
        % Positive for scenes(object), negative for optical images(images).
        val = scene.distance;
        if ~isempty(varargin)
            val = val  *  ieUnitScaleFactor(varargin{1});
        end

    case {'wangular', 'widthangular', 'hfov', 'horizontalfieldofview', ...
            'fov', 'horizontalfov'}
    ...
    case {'dangular', 'diagonalangular', 'diagonalfieldofview'}
        ...
        %
        %  val = sqrt(sceneGet(scene, 'wAngular') ^ 2 ...
        %       + sceneGet(scene, 'hAngular')^2)
        %
    ...
    case 'aspectratio'
        ...
        if isempty(c) || c == 0 ...
                && strcmp(sceneGet(scene, 'type'), 'opticalimage')
    ...
    case {'photons'}
        ...
        % Read photon data. It's possible to ask for just a single waveband
        ...
        if checkfields(scene,'data', 'photons')
            if isempty(varargin)
                ...
                idx = ieFindWaveIndex(sceneGet(scene, 'wave'), ...
                    varargin{1});
    ...
    case {'roiphotons', 'roiphotonsspd'}
        ...
        % So, [col, row, 0, 0] returns 1 point and [col, row, 1, 1] returns
        % 4 points.
        if isempty(varargin)
            error('ROI required')
        else
    ...
    case {'roireflectance'}
        ...
        %
        % Return the reflectance in a region of interest, XW format
        if isempty(varargin)
            error('ROI required');
        else
            roiLocs = varargin{1};
        ...
        switch illF
            case 'spatial spectral'
                % Use the scene tools to extract the relevant portion of
                % the illuminantSPD
    ...
    case {'datamax', 'dmax', 'peakphoton'}
        if checkfields(scene, 'data', 'dmax')
            val = scene.data.dmax;
        else
    ...
    case {'datamin', 'dmin', 'minphoton'}
        if checkfields(scene, 'data', 'dmin')
            val = scene.data.dmin;
        else
            p = sceneGet(scene, 'photons');
            val = min(p(:));
    ...
    case {'wave', 'wavelength'}
        ...
        if checkfields(scene, 'spectrum')
            val = scene.spectrum.wave(:);
        end

    case {'nwave', 'nwaves'}
        if checkfields(scene, 'spectrum')
            val = length(scene.spectrum.wave);
        end
    ...
    case {'width'}
        ...
        % Field of view (horizontal, width)
        w = sceneGet(scene, 'wangular');
        val = 2 * d * tand(w / 2);
        if ~isempty(varargin)
            val = val * ieUnitScaleFactor(varargin{1});
        end
    ...
    case {'diagonal', 'diagonalsize'}
        val = sqrt(sceneGet(scene, 'height') ^ 2 ...
            + sceneGet(scene, 'width') ^ 2);
        if ~isempty(varargin)
            val = val * ieUnitScaleFactor(varargin{1});
        end
    ...
    case {'spatialresolution', 'distancepersample', 'distpersamp'}
        ...
        val = [sceneGet(scene, 'hspatialresolution'), ...
            sceneGet(scene, 'wspatialresolution')];
        if ~isempty(varargin)
            val = val * ieUnitScaleFactor(varargin{1});
        end
    ...
    case {'hangularresolution', 'heightangularresolution'}
        ...
        val = 2 * atand((sceneGet(scene, 'hspatialResolution') ...
            / sceneGet(scene, 'distance')) / 2);

    case {'wangularresolution', 'widthangularresolution'}
        % Angular degree per pixel --
        val = 2 * atand((sceneGet(scene, 'wspatialResolution') ...
            / sceneGet(scene, 'distance')) / 2);

    case {'angularresolution'}
        % Height and width
        val = [sceneGet(scene, 'hangularresolution'), ...
            sceneGet(scene, 'wangularresolution')];
    ...
    case {'illuminantformat'}
        ...
        % Check whether illuminant is spatial spectral or just a SPD vector
        il = sceneGet(scene, 'illuminant');
        if isempty(il)
            disp('No scene illuminant.');
            return;
        else
            val = illuminantGet(il, 'illuminant format');
        end
    ...
    case {'illuminantcomment'}
        if checkfields(scene, 'illuminant', 'comment')
            val = scene.illuminant.comment;
        end
    ...
    case {'rgb', 'rgbimage'}
        ...
        if isempty(varargin)
            ...
            if isempty(sceneW)
                ...
                gam = str2double(get(sceneW.editGamma, 'string'));
            end
        else
            gam = varargin{1};
        end
