...
%  power distribution to the harmonic parameters.  These can be calculated
%  using the humanConeIsolating function (See Example).
%
% Inputs
%   scene  - An initialized scene (sceneInit) or empty
%   params - Struct with harmonic parameters.  
%            Defaults are specifed by the function harmonicP.  See the
%            Examples for parameters
%   wave   - Optional.  Wavelength samples for scene.
%
...
if isfield(params,'backSPD') && isfield(params,'modSPD')
    ...
    photons = contrast(:)*params.modSPD(:)' + repmat(params.backSPD(:)',[r*c,1]);
