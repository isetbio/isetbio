...
%
%    N.B. The source contains an executable examples of usage, which can be
%    accessed by entering 'edit sceneFrequencySupport.m' in MATLAB's
%    command window.
%
% Inputs:
%    scene    - A scene structure
%    units    - (Optional). Frequency units. Default is 'cyclesPerDegree'
%
% Outputs:
...
if isempty(fovHeight) || isempty(fovWidth)
    % There is no scene spatial resolution. This happens only when the
    % scene is empty, I think. So, we must make up, from whole cloth, a set
    % of values that will be OK for this angular resolution. This might be
    % a bad idea.
    disp(['sceneFrequencySupport: Making up arbitrary scene angle, ' ...
        '30x30 deg'])
    fovWidth = 30;
    fovHeight = 30;
    nCols = fovWidth * 10;
...
%
% The Nyquist frequency just with respect to the samples is N / 2. If the
% span of the scene is 1 deg, nCols/2 or nRows/2 is the Nyquist frequency
% in cycles/deg.But the scene field of view may differ from one, so we need
% to divide by the true number of degrees.
%
% For example:
%    If you have 100 spatial samples in a single degree, the Nyquist limit
%    runs to 50 cyc/deg. However, if the FOV is 40 deg, the highest spatial
%    frequency is then 50/40 cyc/deg.
