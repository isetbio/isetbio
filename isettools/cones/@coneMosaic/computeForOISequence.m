    ...
    for oiIndex = 1:oiSequence.length
        ...
        for iTrialBlock = 1:numel(blockedTrialIndices)
            ...
            if (oiIndex > 1) && (integrationTimeForFirstPartialAbsorption > 0)
                ...
                absorptionsAllTrials = zeros(size(obj.pattern,1), size(obj.pattern,2), numel(trialIndicesForBlock), 'single');
            end
            ...
            if (integrationTimeForSecondPartialAbsorption > 0)
                ...
                emSubPath = reshape(squeeze(emPaths(trialIndicesForBlock,idx,:)), [numel(trialIndicesForBlock) 2]);
                currentSeed = currentSeed  + 1;
                absorptionsAllTrials =  absorptionsAllTrials + single(obj.compute(...
                    currentOI, ...
                    'theExpandedMosaic', theExpandedMosaic, ...
                    'seed', currentSeed, ...
                    'emPath', emSubPath, ...
...
%% Nested function for returning absorptions from the last trial
    function returnAbsorptionsFromLastTrial()
        if (isa(obj, 'coneMosaicHex'))
            ...
            % Return the absorptions from the last trial after reshaping to full 3D matrix [cone_rows, cone_cols, time]
