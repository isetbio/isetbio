...
% NPC, ISETBIO Team, 2016  
    ...
    for coneMosaicIndex = 1:numel(coneMosaics)
        coneIndices(coneMosaics{coneMosaicIndex}) = find(obj.pattern == coneMosaicIndex+1);
    end
   
    nFrames = size(responses,3);
    demosaicedResponses = zeros([size(responses,1), size(responses,2) 3 nFrames]);
    ...
    if (isempty(varargin))   
        ...
        % right.
        cornealQuantalEfficienciesEnergyUnits = EnergyToQuanta(obj.wave(:),cornealQuantalEfficiencies);
        ...
        for frameIndex = 1:nFrames
            ...
            LMS = bsxfun(@rdivide, demosaicedResponses(:,:,:,frameIndex), scale);
