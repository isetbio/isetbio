classdef cMosaic < handle
    ...
    %    photocurrent response. The mosaic is typically generated by
    %    cropping a pre-computed, dense, eccentricity-varying lattice of
    %    cone positions.
    %
    ...
    %
    % --- Mosaic Geometry and Position ---
    %
    %    'name'                         - String. Mosaic name. Default 'cone mosaic'.
    %    'eccentricity degs' or 'position degs' - [x,y] vector. The center of the mosaic, in degrees of visual angle.
    %                                       Synonyms: 'position degs' is also accepted. Default: [0 0] (fovea).
    %    'size degs'                    - [sx, sy] vector. Width and height of the mosaic, in degrees of visual angle. Default: [0.4 0.4]
    %    'which eye'                    - String. Which eye the mosaic represents. Valid options are: {'left eye', 'right eye'}. Default: 'right eye'.
    %    'micronsperdegree'             - Scalar. A custom fixed retinal magnification factor (microns/deg) to use for all conversions.
    %    'custom degs to mms conversion function' - Empty [] or function_handle. Function returning eccentricity in retinal mms for eccentricities
    %                                       specified in degs. Used only when generating a mesh from scratch. If specified,
    %                                       'custom mms to degs conversion function' must also be specified.
    %    'custom mms to degs conversion function' - Empty [] or function_handle. Function returning eccentricity in degs for eccentricities
    %                                       specified in retinal mms. Used only when generating a mesh from scratch. If specified,
    %                                       'custom degs to mms conversion function' must also be specified.
    %    'cone data'                    - Struct. Struct containing custom cone position/type data, typically exported from a @coneMosaicHex object.
    %
    % --- Cone Mesh Generation (If 'cone data' is empty) ---
    %
    %    'compute mesh from scratch'    - Logical. If true, a new Delaunay-based mesh is computed for the cone positions. Default: false (uses pre-computed lattice).
    %    'max mesh iterations'          - Scalar. Max number of iterations for the mesh generation (only used if 'compute mesh from scratch' is true). Default: 100.
    %    'visualize mesh convergence'   - Logical. If true, visualize the convergence of the mesh generation (only used if 'compute mesh from scratch' is true). Default: false
    %    'export mesh convergence history'- Logical. If true, save the convergence history of the mesh generation (only used if 'compute mesh from scratch' is true). Default: false
    %    'custom rf spacing function'   - Empty [] or function_handle. Function that returns the RF spacing in microns for an array of RF positions in microns.
    %                                       Used only when generating a mesh from scratch.
    %
    % --- Spectral Properties ---
    %
    %    'wave'                         - Vector. Wavelength support (nm) for the photopigment and macular pigment. Default is: 400:10:700.
    %    'pigment'                      - @cPhotoPigment object. Custom photopigment object.
    %    'macular'                      - @Macular object. Custom macular pigment object.
    %
    % --- Eccentricity-Varying Properties & Scaling ---
    %
    %    'rod intrusion adjusted cone aperture' - Logical or scalar in (0..1]. If true/scalar, adjusts the cone aperture (light collecting area)
    %                                       to account for progressive rod intrusion (only effective if 'ecc varying cone aperture' is true). Default: false
    %    'ecc varying cone aperture'    - Logical. If true, the cone light collecting aperture size varies with eccentricity. Default: true
    %    'ecc varying cone blur'        - Logical. If true, the cone spatial blur varies with eccentricity, using a local average aperture size to define the blur kernel. Default: false
    %    'ecc varying outer segment length' - Logical. If true, the outer segment length (and thus photon capture) varies with eccentricity. Default: true
    %    'ecc varying macular pigment density' - Logical. If true, the macular pigment optical density varies with eccentricity. Default: true
    %    'ecc varying macular pigment density dynamic' - Logical. If true, the macular pigment density correction is updated dynamically based on eye movements. Default: false
    %    'anchor all ecc varying params to their foveal values' - Logical. If true, forces all eccentricity-varying parameters (aperture, outer segment length, macular density)
    %                                       to use their foveal values, effectively turning off all eccentricity variations regardless of other settings. Default: false.
    %
    % --- Cone Type Distribution ---
    %
    %    'conedensities'                - Vector. Contains the relative densities for L-, M-, S-, and possibly a 4th (K-) cone.
    %                                       Must be a 3- or 4-element vector (e.g., [L M S K]). Default: [0.6 0.3 0.1 0.0]
    %    'tritanopic radius degs'       - Scalar. Radius (degrees) of the S-cone free region around the fovea. Default: 0.15 degs
    %
    % --- Timing and Noise ---
    %
    %    'integration time'             - Scalar. The time interval for accumulating photon absorptions, in seconds. Default: 5/1000 (5 ms)
    %    'noise flag'                   - String. Specifies the type of photon noise. Valid options are: {'random', 'frozen', 'none'}. Default: 'random'
    %    'random seed'                  - Scalar. Seed for the random number generator, used for 'random' or 'frozen' noise. If 'frozen' is specified and this is empty, an error is thrown.
    %
    % --- Computation Parameters ---
    %
    %    'use parfor'                   - Logical. If true, use parallel processing (parfor) in applicable functions. Default: true.
    %    'optical image position degs'  - String or [x,y] vector. Specifies how the input optical image is positioned relative to the mosaic during computation.
    %                                       Default: 'mosaic-centered', which centers the OI on the mosaic's eccentricityDegs.
    %    'cone coupling lambda'         - Scalar. Factor ($\lambda$) used to determine the spatial constant for electrical coupling between cones.
    %                                       The spatial constant $\tau$ will be $\lambda \times \text{cone diameter}$. Default: []. If empty, no coupling is applied.
    %    'cone aperture modifiers'      - Struct. Structure to customize how the cone aperture diameter is calculated, including fields like 'smoothLocalVariations' (logical)
    %                                       or 'shape' ('Pillbox', 'Gaussian'). Default: struct('smoothLocalVariations', true)
    %    'cone diameter to spacing ratio'- Scalar in (0..1]. Ratio of cone inner segment diameter to cone spacing (center-to-center distance). Default: 1.0
    %
    % See Also:
    %    cPhotoPigment
    %
