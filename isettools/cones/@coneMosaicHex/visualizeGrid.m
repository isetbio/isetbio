...
if (isempty(axesHandle))
    ...
    cla;
    set(hFig, 'Position', figPosition, 'Color', [1 1 1]);
...
switch showConeDensityContour
    ...
    case 'none'
        % Empty?
    otherwise
        error(['coneMosaicHex.visualizeGrid: ''coneDensityContour' ...
            'Overlay'' must be set to one of the following: ''measured' ...
            ''', ''theoretical'', ''none''. ']);
end

if (showHexMesh)
        % Superimpose hex mesh showing the locations of perfect hex grid
        meshFaceColor = [0.8 0.8 0.8];
        meshEdgeColor = [0.5 0.5 0.5];
        meshFaceAlpha = 0.0;
        meshEdgeAlpha = 0.5;
        lineStyle = '-';
        coneMosaicHex.renderHexMesh(axesHandle, hexCoords(:, 1), ...
            hexCoords(:, 2), meshEdgeColor, meshFaceColor, ...
...
if (~showCorrespondingRectangularMosaicInstead)
    ...
    if (~isempty(outerApertureOutline))
        coneMosaicHex.renderPatchArray(axesHandle, ...
            outerApertureOutline, sampledHexMosaicXaxis(iCols), ...
            sampledHexMosaicYaxis(iRows), edgeColor, faceColorOuter, ...
            lineStyle);
    end
    if (~isempty(innerApertureOutline))
        coneMosaicHex.renderPatchArray(axesHandle, ...
            innerApertureOutline, sampledHexMosaicXaxis(iCols), ...
            sampledHexMosaicYaxis(iRows), edgeColor, faceColorInner, ...
            lineStyle);
    end
...
switch showConeDensityContour
    case 'measured'
        ...
        [cH, hH] = contour(axesHandle, densityMapSupportX, ...
            densityMapSupportY, densityMapMeasured, contourLevels, ...
            'LineColor', 'r', 'LineWidth', 2.0, 'ShowText', 'on', ...
            'LabelSpacing', 2000);
    ...
    case 'theoretical'
        ...
        [cH, hH] = contour(axesHandle, densityMapSupportX, ...
            densityMapSupportY, densityMapTheoretical, contourLevels, ...
            'LineColor', 'b', 'LineWidth', 2.0, 'ShowText', 'on', ...
            'LabelSpacing', 2000);
        clabel(cH, hH, 'FontWeight', 'bold', 'FontSize', 16, ...
    ...
    case 'theoretical_and_measured'
        ...
        contour(axesHandle, densityMapSupportX, densityMapSupportY, ...
            densityMapMeasured, contourLevels, ...
            'LineColor', [1 0.7 0.7], 'LineWidth', 4.0, ...
            'ShowText', 'on', 'LabelSpacing', 2000);
        [cH, hH] = contour(axesHandle, densityMapSupportX, ...
            densityMapSupportY, densityMapMeasured, contourLevels, ...
        ...
        contour(axesHandle, densityMapSupportX, densityMapSupportY, ...
            densityMapTheoretical, contourLevels, ...
            'LineColor', [0.7 0.7 1.0], 'LineWidth', 4.0, ...
            'ShowText', 'on', 'LabelSpacing', 2500);
        [cH, hH] = contour(axesHandle, densityMapSupportX, ...
            densityMapSupportY, densityMapTheoretical, contourLevels, ...
...
if (isempty(p.Results.axesHandle))
    ...
    set(axesHandle, 'XTick', xTicks, 'YTick', yTicks, ...
        'XTickLabel', xTickLabels, 'YTickLabel', yTickLabels);
    set(axesHandle, 'FontSize', 16, 'XColor', [0 0 0], ...
        'YColor', [0 0 0], 'LineWidth', 1.0);
    box(axesHandle, 'on');
    grid(axesHandle, 'off');
    title(axesHandle, sprintf('%2.0f microns', obj.width * 1e6), ...
        'FontSize', 16);
    set(axesHandle, 'XLim', [sampledHexMosaicXaxis(1) - 1.5 * 1e-6, ...
        sampledHexMosaicXaxis(end) + 1.5 * 1e-6]);
    set(axesHandle, 'YLim', [sampledHexMosaicYaxis(1) - 1.5 * 1e-6, ...
        sampledHexMosaicYaxis(end) + 1.5 * 1e-6]);
    drawnow;
end
