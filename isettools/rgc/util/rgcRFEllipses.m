function [sRFcenter, sRFsurround, cellLocation, ellipseMatrix] = rgcRFEllipses(cellLocation,spread,varargin)
...
p.addRequired('cellLocation',@isnumeric); % in units of bipolar samples
p.addRequired('spread',@isscalar);        % 1 std. in units of bipolar samples

% JRG has a theoretical argument about this and we will fill it in.
% But for now, these are hard coded numerical defaults that satisfy the
...
r                   = p.Results.centerSurroundSizeRatio;  % radius ratio between center and surround for DoG
k                   = p.Results.centerSurroundAmpRatio;   % scaling of magnitude of surround
...
cellLocation = cellLocation + centerNoiseBipolars*randn(nRows,nCols,2)*spread;
