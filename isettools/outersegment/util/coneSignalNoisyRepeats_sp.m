function [noisyConeCurrent] = coneSignalNoisyRepeats_sp(ConeCurrentSignal, reps_max, coneSamplingRate, cone_mosaic)
...
%     [sz1 sz2 sz3] = size(ConeSignalFinal_rs);
    noisyConeCurrentRS(noise_reps,1:sz1*sz2,1:sz3) = (ConeSignalFinal_rs);
%     noisyM(noise_reps,1:sz1*sz2,1:sz3) = reshape(ConeSignalFinalCellPlusNoise{2},sz1*sz2,sz3);
%     noisyS(noise_reps,1:sz1*sz2,1:sz3) = reshape(ConeSignalFinalCellPlusNoise{3},sz1*sz2,sz3);
    
    
    
end

noisyConeCurrent = reshape(noisyConeCurrentRS, sz1*sz2, reps_max*sz3);
