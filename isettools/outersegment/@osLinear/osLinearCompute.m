...
%         obj.ConeCurrentSignal = real(ifft((Filter_block2) .* fft(isomerizations,[],3)),[],3) / coneSamplingRate;
        ConeCurrentSignalTemp = real(ifft((Filter_block2) .* fft(isomerizations,[],3)./1,[],3)) * 1;
        % NEED TO CHECK AMPLITUDE
        
        % reshape signal matrix
        cone_locations = find(cone_mosaic==cone_type);
        [cone_locations1, cone_locations2] = find(cone_mosaic==cone_type);
        ConeSignal_rs = reshape(ConeCurrentSignalTemp,[sz1*sz2],nSteps);
