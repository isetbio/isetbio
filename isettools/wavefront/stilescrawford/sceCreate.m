...
%   rho_source       -        Data source for rho
...
%
% Sources for center position 
%   'centered'          -     0,0 mm.  This may be as good a guess as anything, if you don't know R/L eye or anything
%                             about yoru subject (default).
%   'applegate'         -     Right eye numbers from Applegate & Lakshminaranayan, 1993.
%
% Output
%   sceP.wavelengths     SCE wavelengths.  
%   sceP.rho -           SCE peakedness rho as a function of wavelength (units: 1/mm^2)
%   sceP.xo  -           SCE x center position in mm relative to pupil center
%   sceP.yo              SCE y center position in mm relative to pupil center
%
...
%
% The 'applegate' xo, yo parameters are from Applegate & Lakshminaranayan, "Parametric representation
% of Stiles-Crawford functions: normal variation of peak location and directionality", 1993, JOSA A, 1611-1623.
%
...
%   ii) Could allow unit specification rather than default mm-based units.
%   iii) Understand in more detail dependence of SCE on other things, such as
%   field size and position.
%   iv) Is the additive correction of the Berendschot numbers (as opposed to, say,
%   multiplicative) appropriate?
%
% Examples:
%    sceCreate
...
% 8/19/12  dhb  Expanded data options, better comments about source of data.
%
% (c) WVF Toolbox Team 2011-2012

%% Parameter setup
if ~exist('wave','var')   || isempty(wave), wave = (400:10:700)';  end
if ~exist('rho_source','var') || isempty(rho_source), rho_source = 'none'; end
if ~exist('position_source','var') || isempty(position_source), position_source = 'centered'; end
wave = wave(:);
...
switch (position_source)
    case 'centered'
        % Given individual variation, just assuming the center
        % of the pupil is reasonable if you know nothing about
        % the individual subject.
        sceP.xo=0;   % SCE center in mm
        sceP.yo=0;   % SCE center in mm
        
    case 'applegate'
        % These values are from  Applegate & Lakshminaranayan, 1993.
        % They give the horizontal position as nasal in mm, so that
        % the 0.51 number is correct for the right eye (OD) in the
        % OSA coordinate system.  It would be -0.51 for the left eye (OS).
        % The verical position is 0.20 superior, which is positive in
        % the OSA system for both eyes.  Heidi says that in general,
        % people think that 0 might be a better population guess for
        % vertical.
        %
