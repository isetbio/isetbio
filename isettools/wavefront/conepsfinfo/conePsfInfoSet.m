...
%               'wavelengths' Column vector of wavelengths.  This will
%                             spline exant spectral sensitivities and
%                             weighting on set.
...
%                              wavelength samples.  Spectral sampling must
%                              match that specified by wavelengths.
%               'spectralWeighting' Spectral weighting vector, in a column
%                              vector that sums to 1.  This will do the
%                              normalization on set, if it isn't already
%                              normalized. Spectral sampling must
%                              match that specified by wavelengths.
%               'coneWeighting' Cone weighting vector, in a column vector
%                              with one entry per cone class that sums to
%                              1. This will do the normalization on set, if
%                              it isn't already normalized.
...
%{
    ...
    conePsfInfo = conePsfInfoSet(conePsfInfo,'spectralSensitivities',temp.T_cones_sp);
    ...
    case 'wavelengths'
        ...
        if (~isempty(conePsfInfo.spectralSensitivities))
            conePsfInfo.spectralSensitivities = ...
                SplineCmf(conePsfInfo.wavelengths,conePsfInfo.spectralSensitivities,val);
            conePsfInfo.spectralSensitivities = ...
                conePsfInfo.spectralSensitivities/sum(conePsfInfo.spectralSensitivities);
        end
        if (~isempty(conePsfInfo.spectralWeighting))
            conePsfInfo.spectralWeighting = ...
                SplineSpd(conePsfInfo.wavelengths,conePsfInfo.spectralWeighting,val);
            conePsfInfo.spectralWeighting = ...
