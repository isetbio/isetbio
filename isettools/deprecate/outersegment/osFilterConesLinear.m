...
%   newIRFs = osFilterConesLinear(coneMosaic);
% 
...
TauP = 0.5311;   % Period
Phi = 34.1814;   % Phase
Filter = ScFact .* (((TimeAxis./TauR).^3)./(1+((TimeAxis./TauR).^3))) ...
    .* exp(-(TimeAxis./TauD)) .* cos(2*pi*TimeAxis./TauP+ deg2rad(Phi));
...
Ib = [7131 6017 1973]; % R* per sec due to background adapting field

stimNormCoeff = meanIsomerization/max(Ib);

% Set gain parameters according to Angueyra & Rieke:
Ib = Ib*stimNormCoeff;
gain_dark = 0.22;              % from Juan's paper (approximate peak of the IRF measured in darkness, and in units of pA/R*)
