function rect = ISOFindSlantedBar(edgeImage,blurFlag)
%Return a rect containing a good portion of the ISO 12233 slanted bar image
%
%  rect = ISOFindSlantedBar(edgeImage,[blurFlag = false])
%
% Inputs
%  edgeImage: Image (RGB or mono) a slanted edge in it
%  blurFlag:  Apply blurring to image prior to finding rect (default = false)
%
% Return
%  rect:    [upperLeftRow upperLeftCol width height]
%
% This routine finds a rectangle ROI in the slanted edge that will be a
% good choice for sending in to the ISO12233 function.
%
% The rect must be taller than wide and should incorporate enough rows to
% calculate the MTF.
%
% This routine requires more checking. It is not a great algorithm.
%
...
if notDefined('blurFlag'), blurFlag = true; end
...
if blurFlag
    sz  = size(edgeImage); s = round(min(sz)/10);
    g   = fspecial('gauss', s,round(s*0.7));
    edgeImage  = conv2(edgeImage,g,'same');
    blackBorder = false;
end
...
ySums  = sum(edgeImage,2);    % vcNewGraphWin; plot(ySums)
...
dxRowMin = diff(sum(edgeImage(rowMin+(2:5),:)));   % vcNewGraphWin; plot(dxRowMin);
dxRowMax = diff(sum(edgeImage(rowMax-(2:5),:)));   % vcNewGraphWin; plot(dxRowMax);
