...
%   'whatsi',whatsievalue       - Set the whatsi parameter to the value in whatsivalue(default, 10)
%   'whosi','whosiname'       - Set the whosi parameter to the string in whosiname (default, 'joe')
%
% The code illusrates how to check that the passed 'what' string
% corresponds to an allowable value.
%
% The code below also shows how to check whether passed 'unitstring' is within a
% predefined set of allowable unit strings.  You can get very sophisticated
% about argument checking if you like.  See the Matlab documentation on
% inputParser, vaidatestring, and validateattributes. 
%
...
% this and use the default units for that field if this is the string.  If
% some other units are explicitly specified, then the set code would make
% sure that the units were allowable for its field and do the conversion to
% the default units.  Since a single set function may set some parameters
% that are length and others that are time, etc. the parser should probably
% do only a very broad check of allowable units, or perhaps none at all.
%
% In some early iset/isetbio functions, we allowed synonyms for some keys,
% and also allowed keys to have whitespace.  I am not sure we want to
% continue with that, I prefer to just have one key for each attribute, and
% I find it harder to read the high level code when I don't know whether
% two different sets are really doing the same thing or are doing different
% things.
% 
% But if you wanted to do that, you could do so by pre-processing varargin
% to map from the various synonyms onto the standard form understood by the
% parser.  And then the set code would just know about the standard names
% of the resulting parameters fields.
%
% It is not clear how often we'll want key/value options other than units
...
allowableUnitStrings = {'defaultunits', 'nm', 'um', 'mm' 'cm' 'm', 'usec' 'msec' 'sec'};

% Set up key value pairs
%
% Defaults are given here, as is a type check function
p.addParameter('units','defaultunits',@(x) any(validatestring(x,allowableUnitStrings)));
