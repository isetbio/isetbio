classdef oiSequence < handle
    ...
    %
    % Description:
    %    The stimuli for many psychophysical and physiological experiments
    %    can be described as a fixed background image combined with a
    %    time-varying image. The oiSequence class includes methods that
    %    combine the such image combinations across time.
    %
    %    There is a companion method, oisCreate() that produces certain
    %    classic oiSequences (harmonics, vernier targets, impulses). For
    %    some cases, that function may provide a simpler interface for
    %    creating an oiSequence.
    %
    ...
    %    oiModulated        - Struct.
    %    oiTimeAxis         - Numeric.
    %    modulationFunction - Numeric.
    %
    % Outputs:
    %    The created OI Sequence object.
    %
    % Optional key/value pairs:
    %    'composition'      - String. Select one of the appropriate strings
    %                         to indicate how to handle the fixed &
    %                         modulated OIs. The options are: 'add',
    %                         'blend', and 'xor'.
    %    'modulationRegion' - Struct. A select region of interest to be
    ...
    properties (SetAccess = private)
        ...
        %   Whether to add the oiModulated to the oiFixed or to blend it
        %   with the oiFixed.
        composition;
    ...
    methods  % public methods
        ...
        function obj = oiSequence(oiFixed, oiModulated, oiTimeAxis, ...
            ...
            %
            % Optional key/value pairs:
            %    modulationRegion   - Struct.
            %    composition        - String. One of 'add', 'blend', or
            %                         'xor'. Default 'add'.
            ...
            % The timeAxis must be the same length as the number of values
            % in the modulationFunction. If it only has 1 value, we are
            % going to assume that the value is delta T and we will create
            % the whole vector. If it has a vector, but that vector is not
            % the same length as the modulationFunction, we throw an error.
