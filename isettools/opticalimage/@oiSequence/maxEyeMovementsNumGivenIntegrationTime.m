function maxEyeMovementsNum = maxEyeMovementsNumGivenIntegrationTime(...
    ...
    %
    % Description:
    %    Generate the eye movement sequence for all of the optical images.
    %
    %    Method to compute the maximum number of eye movements given an
    %    oiSequence and a mosaic's intergration time. The optional
    %    parameter stimulusSamplingInterval' is only used when the
    %    oiSequence has a length of 1, in which case we do not have
    %    information regrading the duration of that single frame.
    %
    % Inputs:
    %    obj                - Object. An OI sequence object.
    %    integrationTime    - Numeric. The integration time.
    %    varargin           - (Optional). Additional parameters in
