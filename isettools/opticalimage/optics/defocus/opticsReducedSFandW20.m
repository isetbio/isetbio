function [alpha, rSF, w20] = opticsReducedSFandW20(p, D0, sampleSF, wave)
% Compute the human optical transfer function
%
% Syntax:
%   [alpha, rSF, w20] = opticsReducedSFandW20(optics, wave, dist)
%
% Description:
%    The derivation of the parameters needed for the defocus OTF are
%    computed here.
%
% Inputs:
...
%    wave     - Numeric. Wavelength in nanometers
%
% Outputs:
%    otf      - Vector. Optical transfer function (actually, this is the
...
%
% Notes:
%    * TODO: We need a form of this same function that calls
%      opticsDefocusedMTF for a lens imaging a plane that is out of the
%      good focal distance. Then, if we have a depth map, we compute the
%      defocus (D) & apply this formula - without using the williamsFactor.
%    * [Note: JNM - There is no checking for any of the variables? Update:
%      I changed the heading of the function to match what the body
%      expects in terms of variables. I did not add checking though, so if
%      that is something we need please let me know.]
%    * TODO:
%      = Use the Marimont paper to get more references for this routine.
%      = There are comments below about the Hopkins formula. These should
%        be moved to a wiki page or perhaps the routine opticsDefocusedMTF.
%
