...
%    otf      - Matrix. Optical transfer function matrix, otf(wave, sf)
...
%
% Notes:
%    * TODO: investigate if  there needs to be a DC term below (see comment
%      ~L57 for clarification).
%
% See Also:
%    customOTF, humanOTF
%
...
%{
    ...
    defocus = zeros(size(opticsGet(optics, 'wave')));
...
% Frequency representation in optics structure. Units are cyc/mm
...
%
% N.B. This spatial frequency gets too high sometimes and can slow down the
% calculation. The maxF may be, say 1000 cyc/mm in which case we would need
% 2 samples/micron to measure at that level. Pixels are only about 2
% microns, maybe 1 micron. Not sure what to do here. Could just leave it or
% we could think more.
%
...
for ii = 1:nWave
    ...
    % sampleSF in c/mm. otf(ii, :) calculated in opticsDefocusedMTF effSF
    % is the effective spatial frequency of the support in c/mm
