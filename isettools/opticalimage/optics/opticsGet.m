...
switch parm
    ...
    case {'imagedistance', 'focalplane', 'focalplanedistance'}
        ...
        if isempty(varargin), sDist = Inf; else, sDist = varargin{1}; end
        val = 1 / (1 / fL - 1 / sDist);   % Lens maker equation

        % These all need the scene field of view. They can be computed from
        % the geometry of the image distance and FOV.
    case {'imageheight'}
    ...
    case {'diffractionlimitedpsfdata'}
        ...
        % some number of spatial samples, and some amount of oversampling
        % on the frequency calculation to make the curve smooth.
        %
        % * [Note - DHB: The frequency support for this is what you get by
        % calling fopticsGet(optics, 'dl fsupport matrix', thisWave, units,
        % nSamp) and then multiplying by frequencyOverSample.]
        if isempty(varargin), error('You must specify wavelength');
        else, thisWave = varargin{1};
        end
        if length(varargin) < 2, units = 'um';
        else, units = varargin{2}; end
        if length(varargin) < 3, nSamp = 100;
        else, nSamp = varargin{3}; end
        if length(varargin) < 4, freqOverSample = 1;
        else, freqOverSample = varargin{4}; end

        %  Oversample the frequency to get a smoother PSF image. You can
        %  specify the factor for oversampling in the calling arguments.
    ...
    case {'distperdeg', 'distanceperdegree'}
        ...
        %------------------------
        % [Note: XXX - I think we should delete the stuff below and the
        % OTF.fx entry, using only the oiFrequencySupport call. This will
        % require debugging, particularly in the script s_TestSI and
    ...
    case {'psfdata'}
        ...
        switch lower(oModel)
            case 'diffractionlimited'
                ...
                % Number of frequency steps from 0 to incoherent cutoff
                nSamp = 100;
                dlF = opticsGet(optics, 'dlFSupport', ...
                    thisWave(1), units, nSamp);
                [fSupport(:, :, 1), fSupport(:, :, 2)] = ...
                    meshgrid(dlF{1}, dlF{2});
                % This's a way to calculate the spatial support for the psf
                % See 'psfsupport' below, also. These should be integrated
                % and coordinated with fSupport.
                % If we would like the spatial support to be smaller and
                % finer, we should scale fSupport * 4, above.
                %   samp = (-nSamp:(nSamp - 1));
    ...
    case {'psfspacing'}
        ...
        % Peak frequency is in cycles/meter. 1 / peakF is in meters. We
        % have two samples in that distance, so the sample spacing is half
        % that distance.
        val = 1 / (2 * peakF);

    case {'psfsupport'}
        % opticsGet(optics, 'psf support', unit) Returns mesh grid of X and
        % Y values. Used for mesh plotting often. X/Y could be mixed up in
        % 1 and 2.
        %
        % [Note: DHB - There was a comment that the code here should be
        % updated to match what is in oiPlot/plotOTF, 'psf 550' case. I did
        % this change, because the way it was caused some problems and
        % confusion for me. In the new code, 0 position will show up
        % explicitly in the list of positions, not true of the old code.]
        ...
        % comment above that it should be replaced. Below's the
        % replacement. It does the same thing for odd dimensional support,
        % & I think now does the right thing for even dimensional support.
        %
