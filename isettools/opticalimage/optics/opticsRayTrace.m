...
%
% Description:
%    Use ray trace information, say generated by Code V or Zemax lens
%    design programs, to compute the optical image irradiance. See the
%    routines rtGeometry and rtApplyPSF for the algorithms applied.
%
% Inputs:
%    scene - (Optional) Struct. A scene structure. Default uses vcGetObject
%            to retrieve an existing scene.
%    oi    - (Optional) Struct. An optical image structure. Default uses
%            vcGetObject to retrieve an existing oi.
%
% Outputs:
...
%
% See Also:
%    oiCompute, rtPrecomputePSFApply, rtPrecomputePSF
%
...
if isempty(opticsGet(optics, 'rayTrace'))
    ...
    ieInWindowMessage(...
        'Use Optics | Import to get ray trace information.', handles, 3);
    %delete(wBar);
    return;
end
...
if sceneFOV > rtFOV
    str = sprintf('Scene diag fov (%.0f) exceeds max RT fov (%.0f)', ...
        sceneFOV, rtFOV);
...
if rtDist ~= sceneDist
    delay = 1.5;
    str = sprintf(['Scene distance (%.2f m) does not match ray trace ' ...
        'assumption (%.2f m)'], sceneDist, rtDist);
...
% figure;
% img = imageSPD(irradiance, oiGet(oi, 'wave'));
% imagesc(img)
