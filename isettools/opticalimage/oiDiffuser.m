...
% so that we don't have truncation problems.  If the support is larger than
% the image, the processing time is very slow.  Also, the condition is
...
hsize = ceil(8*sigma); oiRows = oiGet(oi,'rows');
if hsize >= oiRows % 0.5*oiRows,
    hsize = oiRows;
    % warning('Diffuser not applied. Spread too large.'); return;
elseif ~isodd(hsize), hsize = hsize+1;
end
