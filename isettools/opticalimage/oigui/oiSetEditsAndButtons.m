...
%
% Inputs:
%    handles - Handle. Handle to the optical image's window.
%
% Outputs:
%    None.
%
...
switch lower(dMethod)
    ...
    case 'blur'
        ...
        set(handles.txtBlurSD, 'TooltipString', ...
            'Full width half maximum of Gaussian spread');
    ...
    case 'birefringent'
        ...
        set(handles.txtBlurSD, 'TooltipString', ...
            'Birefringent displacement in microns');
    otherwise
...
% See oiShowImage. In the future, we will read the displayFlag from either
% a global or a setting in the oi GUI.
...
% Visibility switch for buttons/fields in the display.
%
% Syntax:
%   switchControlVisibility(handles, state, model)
%
% Description:
...
%    model   - String. One of {'shiftInvariant', 'diffraction', 'iset3d'}.
%
% Outputs:
%    None.
%
...
switch state
    ...
    case 'off'
        ...
        % Turn stuff off a little differently from the various models Not
        % quite sure we have the right conditions here. For diffraction, I
        % think it is always on. For iset3d we have none of the options,
        % but for shift invariant, we might have some of the other
        % calculations (off axis, anti-alias). Not sure. (BW).
        switch model
            case 'shiftinvariant'
            case 'diffraction'
            case 'iset3d'
