...
%
%    We combine the irradiance data by superimposing the nearer ones on top
%    of the farther ones. This algorithm has imperfections we are trying to
%    improve upon.
%
% Inputs:
%    oiD - Cell Array. A cell array of OI's to combine according to the
%          method specified in the description. Note that the OI's in the
%          oiD should be ordered from nearest to farthest according to the
%          depth plane.
%
% Outputs:
%    oi  - Struct. The combined optical image structure.
%
...
for ii = 1:nDepths, dMaps(:, :, ii) = oiGet(oiD{ii}, 'depthMap'); end
...
% dMap = oiGet(oiD{nDepths}, 'depth map');
% % figure;
% % imagesc(dMap)
% for ii = 1:nWave
