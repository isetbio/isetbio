function temporalEquivalentEccDegs = temporalEquivalentEccentricityForEccXYDegs(obj, xyEccDegs)

    if (size(xyEccDegs,2) ~= 2)
        error('xyEccDegs must be an N x 2 matrix of (x,y) eccentricities.');
    end

    % Find which RGCs are in the nasal reridian
    horizontalEcc = xyEccDegs(:,1);
    switch (obj.whichEye)
        case 'left eye'
    ...
    switch (obj.temporalEquivantEccentricityScalingMode)
    ...
    temporalEquivalentEccDegs = xyEccDegs;
    ...
    if (debugConversion)
        figure();
        subplot(1,2,1)
        plot(xyEccDegs(:,1), temporalEquivalentEccDegs(:,1), 'r.');
        axis 'square'
        set(gca, 'XLim', [-1 1], 'YLim', [-1 1])
        subplot(1,2,2)
        plot(xyEccDegs(:,2), temporalEquivalentEccDegs(:,2), 'b.');
        axis 'square'
        set(gca, 'XLim', [-1 1], 'YLim', [-1 1]);
    end
