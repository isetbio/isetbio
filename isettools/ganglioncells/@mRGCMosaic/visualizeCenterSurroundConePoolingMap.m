function [hFig, ax, centerLineWeightingFunctions, surroundLineWeightingFunctions] = visualizeCenterSurroundConePoolingMap(obj, theRGCindex, varargin)
    ...
    if (~isempty(theSuperimposedPSF))
        % Generate a PSF visualization data struct (containing the vLambda-weighted PSF) for
        % visualization purposes
        % Exactly centered

        PSFvisualizationOffset = obj.eccentricityDegs - [mean(domainVisualizationLimits(1:2)) mean(domainVisualizationLimits(3:4))];
        % Put it on the corner so we can see the RF center
        PSFvisualizationOffset = PSFvisualizationOffset - 0.25*[diff(domainVisualizationLimits(1:2)) diff(domainVisualizationLimits(3:4))];
