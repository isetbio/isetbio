function [oneSidedSpatialFrequencySupport, oneSidedSTF] = ...
    spatialTransferFunction(spatialSupportDegs, theLineWeightingFunction)

    fftSize = 2^(2+(nextpow2(numel(theLineWeightingFunction))));
    dx = spatialSupportDegs(2)-spatialSupportDegs(1);
    maxSF = 1/(2*dx);
    deltaSF = maxSF / (fftSize/2);
    spatialFrequencySupport = ((-maxSF+deltaSF):deltaSF:maxSF);

    theAmplitudeSpectrum = fftshift(abs(fft(theLineWeightingFunction, fftSize)));
