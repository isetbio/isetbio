function [RsToRcVarianceCK, intStoCsensVarianceCK, RsToRcVariance, intStoCsensVariance, ...
	...
	RGCMosaicAnalyzer.visualize.doubleScatterPlot(figNo, ...
		...
		[minEccSupportDegs maxEccSupportDegs], [0.01 0.03 0.1 0.3 1 3 10 30], [0 maxRcDegs], 0:0.05:0.25,...
		'log', 'linear', ...
	...
	maxRsRcRatio = 20;
	populationContourInsteadOfPointCloud.ySupport = linspace(0, maxRsRcRatio, 50)
	...
	RGCMosaicAnalyzer.visualize.doubleScatterPlot(figNo, ...
		...
		's', 16, [0.8 0.8 0.8], 0.8, 1.0, [], ...
		[minEccSupportDegs maxEccSupportDegs], [0.01 0.03 0.1 0.3 1 3 10 30], [0 20], 0:2:20, ...
		'log', 'linear', ...
	...
	maxIntStoCsensRatio = 1.6;
	populationContourInsteadOfPointCloud.ySupport = linspace(0, maxIntStoCsensRatio, 50);
    ...
    RGCMosaicAnalyzer.visualize.doubleScatterPlot(figNo, ...
		...
		's', 16, [0.8 0.8 0.8], 0.8, 1.0, [], ...
		[minEccSupportDegs maxEccSupportDegs], [0.01 0.03 0.1 0.3 1 3 10 30], [0 maxIntStoCsensRatio], 0:0.2:2, ...
		'log', 'linear', ...
	...
	% Every 0.125
	%intStoCsensBins = linspace(0, 1.5, 13);

	% Every 0.15
	%intStoCsensBins = linspace(0, 1.5, 11);

	% Every 0.2
	intStoCsensBins = linspace(0, 1.6, 9);
...
function dataOut = fitTheSTF(theMRGCMosaic, theRGCindex, stimParams, ...
	...
	if (visualizeModelFitting)
		...
		NicePlot.exportFigToPDF(strrep(thePDFfileName, '.pdf', 'SinusoidalFits.pdf'), hFigSinusoidalFits,  300);
	end
	...
	if (~isempty(mRGCNonLinearityParams))
        ...
        if (~isempty(customTemporalFrequencyAndContrast))
        	...
        	stimulusPostFix = sprintf('%s%2.0f_x%1.0f_%02.0fHz', ...
                stimulusPreFix, ...
                customTemporalFrequencyAndContrast.achromaticContrast*100, ...
