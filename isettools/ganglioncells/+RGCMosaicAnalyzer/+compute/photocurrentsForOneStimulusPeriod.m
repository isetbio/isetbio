...
function [temporalSupportPhotocurrent, theConePhotocurrents, theConeBackgroundPhotocurrents, ...
    ...
    p.addParameter('onlyKeepResponseDuringLastTwoStimulusPeriods', false, @islogical);
    p.addParameter('computePhotocurrentResponsesOnlyForSelectConeIndices', [], @isnumeric);
    % Execute the parser
    p.parse(varargin{:});
    onlyKeepResponseDuringLastTwoStimulusPeriods = p.Results.onlyKeepResponseDuringLastTwoStimulusPeriods;
    ...
    temporalSupportPhotocurrent = computeSingleConePhotocurrentResponse( ...
        ...
        pCurrentTemporalResolutionSeconds, [], false);
    ...
    if (debugInputConeMosaicPcurrentResponse)
        ...
        parfor iConeIndex = 1:nCones
            ...
            [~, theConePhotocurrents(:, iConeIndex), ...
                ...
                thePeriodicConeMosaicExcitations(iConeIndex,:), ...
...
function [temporalSupportPhotocurrent, ...
    ...
    thePcurrentBackgroundResponseTransient, ...
    theConeOSbiophysModel] = computeSingleConePhotocurrentResponse(...
    ...
    [thePcurrentDifferentialPeriodicResponse, photocurrentPeriodicResponseTimeAxis, ...
        ...
        cMosaic.photocurrentFromConeExcitationRateUsingBiophysicalOSmodel(...
            ...
            'theConeOSbiophysModel', theConeOSbiophysModel);
    

    backgroundPhotocurrent = thePcurrentBackgroundResponseTransient(end);

    if (onlyKeepResponseDuringLastTwoStimulusPeriods)
        % Only keep pCurrent response during the last 2 stimulus periods
        if (photocurrentPeriodicResponseTimeAxis(end)-2*tOneStimulusCycle <= 0)
            error('Not enough photocurrent response to keep 2 stimulus cycles');
        end
        idx = find(photocurrentPeriodicResponseTimeAxis >= photocurrentPeriodicResponseTimeAxis(end)-2*tOneStimulusCycle);
    ...
    elseif (onlyKeepResponseDuringLastStimulusPeriod)
        % Only keep pCurrent response during the last stimulus period
        if (photocurrentPeriodicResponseTimeAxis(end)-tOneStimulusCycle <= 0)
            error('Not enough photocurrent response to keep 1 stimulus cycle');
        end
