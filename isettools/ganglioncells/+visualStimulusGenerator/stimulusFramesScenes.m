function [theScenes, theNullStimulusScene, spatialSupportDegs, coneFundamentalsStruct] = ...
        stimulusFramesScenes(presentationDisplay, stimParams, spatialModulationPatterns, varargin)

    p = inputParser;
    p.addParameter('validateScenes', false, @islogical);
    p.addParameter('frameIndexToCompute',  [], @isnumeric);
    ...
    frameIndexToCompute = p.Results.frameIndexToCompute;
    ...
    pixelsNum  = round(stimParams.sizeDegs / stimParams.resolutionDegs);
    spatialSupportDegs = linspace(-0.5*stimParams.sizeDegs, 0.5*stimParams.sizeDegs, pixelsNum);
    ...
    if (isempty(frameIndexToCompute))
    ...
    for sceneIndex = 0:nStim
        ...
        if (~isempty(frameIndexToCompute))
            if (sceneIndex ~= frameIndexToCompute)
                continue;
            end
        end
        ...
        % Set the desired FOV 
        theScene = sceneSet(theScene, 'h fov', stimParams.sizeDegs);
...
function visuallyContrastCustomConeFundamentalsToDefaultSS2(customConeFundamentalsStruct)
    ...
    for iFund = 1:3
        subplot(1,3,iFund);
        plot(customConeFundamentalsStruct.spectralSupport, customConeFundamentalsStruct.coneFundamentals(:,iFund), 'ro-');
        hold on;
        plot(customConeFundamentalsStruct.spectralSupport, coneFundamentalsSS2(:,iFund), 'ko-');
        legend({'custom', 'SS2'});
    end
