function axPos = axesMatrixPosition(obj)
    ...
    normalizedPos(:,1) = 0.02+0.92*(obj.rgcRFpositionsDegs(:,1)-mins(1))/(maxs(1)-mins(1));
    normalizedPos(:,2) = 0.02+0.92*(obj.rgcRFpositionsDegs(:,2)-mins(2))/(maxs(2)-mins(2));
    
    rgcsNum = size(obj.rgcRFpositionsDegs,1);
    w = 0.9 * mean(obj.rgcRFspacingsDegs) / (maxs(1)-mins(1));
    h = w;
