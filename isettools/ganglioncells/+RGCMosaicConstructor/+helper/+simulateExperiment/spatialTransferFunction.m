...
function spatialTransferFunction(theMRGCMosaic, theOI, ...
    ...
    if (computeInputConeMosaicResponses)
        ...
        if (computeInputConeMosaicResponsesBasedOnPhotocurrents) && (~isempty(photocurrentParams))
    ...
    if (computeMRGCMosaicResponses)
        % Check if we can write to file
        RGCMosaicConstructor.helper.utils.feasibilityOfWritingToFile(theMRGCMosaicSTFResponsesFullFileName);
...
function computeMRGCMosaicSTF(...
    ...
    if (photocurrentBasedSTFsComputed)
        fprintf('Loading computed input cone mosaic STF responses (cone excitations + photocurrents) from %s.\n', theInputConeMosaicSTFResponsesFullFileName);
        ...
        if (~mRGCsOperateOnBackgroundAdaptedPhotocurrents)
            ...
            for iOri = 1:size(theInputConeMosaicPhotocurrents,1)
                for iSF = 1:size(theInputConeMosaicPhotocurrents,2)
                    theInputConeMosaicPhotocurrents(iOri,iSF,:,:) = bsxfun(@plus, ...
...
function computeInputConeMosaicSTF(theMRGCMosaic, theOI, STFparamsStruct, theInputConeMosaicSTFResponsesFullFileName, ...
    ...
    if (strcmp(theSignal,'cone excitations'))
        ...
        [thePresentationDisplay, ...
         ...
         stimParams.backgroundLuminanceCdM2] = visualStimulusGenerator.presentationDisplay(...
            ...
            'adjustBackgroundChromaticityToEqualizeLandMconeExcitations', STFparamsStruct.adjustBackgroundChromaticityToEqualizeLandMconeExcitations, ...
    ...
    if (strcmp(theSignal, 'photocurrents'))
        ...
        nCones = size(theInputConeMosaicSTFresponses,4);
        ...
        for iSF = 1:numel(stimParams.spatialFrequencyCPD)
            ...
            [theInputConeMosaicPhotocurrentTemporalSupportSeconds, ...
             theInputConeMosaicPhotocurrents(iOri, iSF,:,:), ...
             theInputConeMosaicBackgroundPhotocurrents(iOri, iSF,:), ...
             ~, ~, theConeOSbiophysModels] = RGCMosaicAnalyzer.compute.photocurrentsForOneStimulusPeriod(...
                    ...
                    plotTitle, ...
                    'onlyKeepResponseDuringLastTwoStimulusPeriods', true, ...
