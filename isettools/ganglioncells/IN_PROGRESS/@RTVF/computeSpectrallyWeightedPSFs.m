...
%         
    ...
    [obj.spectrallyWeightedPSFData, ...
     obj.opticsParams] = computeSpecrallyWeightedPSFs(...
            obj.fixedZernikeCoefficientsPositionDegs, ...
            obj.opticsParams, ...
            obj.coneMosaic, ...
            visualize);
end
...
function [thePSFData, opticsParams] = computeSpecrallyWeightedPSFs(...
    fixedZernikeCoefficientsPositionDegs, opticsParams, theConeMosaic, visualize)
    ...
    % Zernike coefficients eccentricity: it can match the opticsParams.positionDegs,
    % or be fixed across all positions in the mosaic
    if (~isempty(fixedZernikeCoefficientsPositionDegs))
        theZernikeCoefficientEccentricityDegs = fixedZernikeCoefficientsPositionDegs;
    else
        theZernikeCoefficientEccentricityDegs  = opticsParams.positionDegs;
    end

    eccDegs = sqrt(sum(theZernikeCoefficientEccentricityDegs .^2,2));
    ...
    [oiEnsemble, psfEnsemble] = theConeMosaic.oiEnsembleGenerate(...
                    theZernikeCoefficientEccentricityDegs , ...
