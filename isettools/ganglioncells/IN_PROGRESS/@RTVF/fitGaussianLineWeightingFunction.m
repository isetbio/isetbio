function theFittedGaussianLineWeightingFunction = ...
    ...
    assert(all(size(spatialSupport) == size(theRFprofile)), 'RTVF: fitGaussianLineWeightingFunction:: mismatch in dimensions');
    
    originalMax = max(theRFprofile(:));
    theRFprofile = theRFprofile / originalMax;
    ...
    RTVF.visualizeFittedModelParametersAndRanges(ax, params);

    % Compute the fitted Gaussian line weighting function
    theFittedGaussianLineWeightingFunction.profile = RTVF.gaussianLineWeightingProfile(params.finalValues, spatialSupport) * originalMax;
    ...
    function rmse = gaussianProfileObjective(params)
        theFittedLineWeightingProfile = RTVF.gaussianLineWeightingProfile(params, spatialSupport);
