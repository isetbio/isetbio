function p = configure(sourceLatticeSizeDegs, neuronType, whichEye)
    ...
    p.latticeGalleryDir = fullfile(isetbioRootPath, 'isettools/ganglioncells/data/lattices');

    % Patch filename (history of positions)
    if (sourceLatticeSizeDegs >= 10)
        p.patchSaveFileName = sprintf('%s_%s_%1.0fdeg_mosaic_progress.mat', ...
            strrep(whichEye, ' ', '_'), strrep(neuronType, ' ', '_'), sourceLatticeSizeDegs);
    else
        p.patchSaveFileName = sprintf('%s_%s_%1.2fdeg_mosaic_progress.mat', ...
            strrep(whichEye, ' ', '_'), strrep(neuronType, ' ', '_'), sourceLatticeSizeDegs);
    end
