...
% convert ctToolbox display structure to isetbio structure
%
% Syntax:
%   function d = displayConvert(ctDisplay, [varargin])
%
% Description:
%    Use this function to convert from the ctToolbox display structure
%    to the isetbio display structure.
%
%  Inputs:
%    ctDisplay  - ctToolbox display calibration file name or display
%                 structure
%    varargin   - (Optional) Additional arguments as needed, such as:
...
%           if filename already exist, whether or not to overwrite file.
%         * display name
%                 
%  Outputs:
%    d          - Struct. ISETBio / ISET display structure
%
...
%% Init
if notDefined('ctDisp')
    error('ctToolbox display structure required');
end
...
s = ctDisp.sPhysicalDisplay.m_objCDixelStructure.m_aSpectrumOfPrimaries;
d = displaySet(d, 'spd', s');
