...
%
% Description:
%    When we calibrate a display with the PsychToolbox, we can then
%    convert the PT format to the display format that is used by ISET.
%    This format is simpler than the PT format and is used by files such
%    as sceneFromFile.
%
% Inputs:
%    fname - String. The psychToolbox display filename.
%    iWave - (Optional) Vector. Wavelength vector. Default is to use the
%            wave from the PT formatted display.
%
% Outputs:
%    d     - Struct. The ISETBIO formatted display structure.
%
% Optional key/value pairs:
%    None.
%
% See Also:
%    sceneFromFile, displayGet
%
% Examples:
%{
    % ETTBSkip - missing filename+
