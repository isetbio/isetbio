...
% Receptive field ensity as a function of visual field location
%
%
%
% Scientific source
%
%   A formula for human retinal ganglion cell receptive field density as a
%   function of visual field location. Andrew (Beau) Watson, Journal of
%   Vision, June 2014, Vol.14, 15. doi:10.1167/14.7.15
%
...
% rm, scale factor for decline in midget fraction with eccentricity	41.03	deg
%
% Item	Value	Unit
% Peak cone density	14,804.6	deg?2
% Peak RGCf density	33,162.3	deg?2
...
for k = 1:4
    ...
    r = [.05:.1:100];
    % dgf = dgf0*[ paramsk(1)*(1+r./paramsk(2)).^(-2) + (1-paramsk(1))*exp(-r./paramsk(3)) ];
    
    dgf(k,:) = dgf0*[ paramsk(k,1)*(1+r./paramsk(k,2)).^(-2) + (1-paramsk(k,1))*exp(-r./paramsk(k,3)) ];
    
    % vcNewGraphWin;
    % hold on
    % plot(r,dgf); set(gca,'xscale','log'); set(gca,'yscale','log');
end
...
set(gca,'fontsize',14);
% 9
smf1d = sqrt(2./(sqrt(3)*dmf1d)); % vcNewGraphWin; plot(r,smf1d); grid on;
...
for x = degarr
    ...
    for y = degarr
        ...
        for k = karr
            kctr = kctr+1;
            dgfE(xctr,yctr,kctr) = dgf0*[ paramsk(k,1)*(1+rxy./paramsk(k,2)).^(-2) + (1-paramsk(k,1))*exp(-rxy./paramsk(k,3)) ];
