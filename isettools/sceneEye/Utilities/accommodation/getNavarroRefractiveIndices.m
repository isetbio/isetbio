...
% Wave should be a vector in nm (e.g. 400,500,...800)
% A (accomodation) should be a value in diopters
...
a = zeros(4,4); % Rows are ocular media, columns are coefficients (e.g. A,D,B,C)
for i = 1:4
    ...
    % [A D B C]
    a(i,:) = [n_curr*(A(:,1)) n_curr*(A(:,2)) n_curr*(A(:,3)) n_curr*(A(:,4))];
...
if(sum(wave < 400 + wave > 800) > 0)
    error('Units for wavelength don''t seem correct...they should be nm here.');
end
...
for i = 1:length(mediaNames)
    RI_all(i,:) = a(i,1) + a(i,2)*wave.^2 + a(i,3)*L + a(i,4)*L.^2;
end
cor = RI_all(1,:);
aqu = RI_all(2,:);
len = RI_all(3,:);
