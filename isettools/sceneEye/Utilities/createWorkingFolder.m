...
% Because we will be modifying the PBRT file and writing out new resource
% files for each rendered image, we need a local "working" folder where
% everything will be stored. This is passed in through 'workingFolder,' or
% is set by default to emRootPath/local/
...
if(isempty(path))
    error('Scene file must be an absolute path.');
end
