...
warning('Deprecated. Use ieLUTInvert instead');
...
% Find the locations where this table is not monotonic
 ...
 if ~isempty(list)
  announce = sprintf('Gamma table %d NOT MONOTONIC',ii);
  disp(announce)
...
%
 result(:,ii) = reshape(interp1(monTable, posLocs-1, ...
                (0:(numEntries-1))/(numEntries-1)), numEntries, 1); 
